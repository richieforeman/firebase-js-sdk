...
describe('generateAuthToken', () => {
  ...
  describe('successful request', () => {
    ...
    it('calls the generateAuthToken server API with correct parameters', async () => {
      ...
      await generateAuthToken(dependencies, registeredInstallationEntry);
  ...
  describe('failed request', () => {
    it('throws a FirebaseError with the error information from the server', async () => {
      ...
      await expect(
        generateAuthToken(dependencies, registeredInstallationEntry)
    ...
    it('retries once if the server returns a 5xx error', async () => {
      ...
      await expect(generateAuthToken(dependencies, registeredInstallationEntry))
        .to.be.fulfilled;
